...
main( int argc, char * argv[] )
{
  ...
  {
    // 1D
    //run( ProcessImage, TransformPrecisionType, Euler1DTransformType, Similarity1DTransformType, float, float, 1 );

    if (Dimension == 2)
    {
          using InputImageType = itk::Image< float, 2>;
          using OutputImageType = itk::Image< float, 2>;
          result = ProcessImage< TransformPrecisionType, Euler2DTransformType, Similarity2DTransformType, InputImageType, OutputImageType >(parameters);
    }
    else if (Dimension == 3)
    {
        using InputImageType = itk::Image< float, 3>;
        using OutputImageType = itk::Image< float, 3>;
        result = ProcessImage< TransformPrecisionType, Euler3DTransformType, Similarity3DTransformType, InputImageType, OutputImageType >(parameters);
    }
...
ProcessImage( const Parameters & _parameters )
{
  ...
  // Filter typedefs
  typedef itk::TransformixFilter< InputImageType > CPUFilterType;
  typedef itk::TransformixFilter< InputImageType > GPUFilterType;
  ...
  if( !_parameters.skipCPU )
  {
    ...
    // Parameter object
    auto parameter_object = elx::ParameterObject::New();

    parameter_object->AddParameterMap({
         {"Spacing", elastix::Conversion::ToVectorOfStrings(outputSpacing)},
         {"Size", elastix::Conversion::ToVectorOfStrings(outputSize)},
         {"Index", elastix::Conversion::ToVectorOfStrings(inputRegion.GetIndex())},
         {"Origin", elastix::Conversion::ToVectorOfStrings(outputOrigin)},
         {"HowToCombineTransforms", {"Compose"}},
         {"InitialTransformParametersFileName", {"NoInitialTransform"}},
         {"Direction", elastix::Conversion::ToVectorOfStrings(outputDirection)},
         {"ResampleInterpolator", {"Final" + _parameters.interpolator + "Interpolator"}},
         {"DefaultPixelValueForGVI", {std::to_string(defaultValue)}},
        });
    ...
    if( !updateExceptionCPU )
    {
      for( unsigned int i = 0; i < _parameters.runTimes; i++ )
      {
        try
        {
          CPUFilter->SetMovingImage( CPUReader->GetOutput() );
          CPUFilter->SetTransform( CPUTransform );
          CPUFilter->SetTransformParameterObject( parameter_object );
        }
        catch( itk::ExceptionObject & e )
        {
  ...
  if( !_parameters.skipGPU )
  {
    ...
    if( !updateExceptionGPU )
    {
      for( unsigned int i = 0; i < _parameters.runTimes; i++ )
      {
        try
        {
          GPUFilter->SetMovingImage( GPUReader->GetOutput() );
          GPUFilter->SetTransform( CPUTransform );
          GPUFilter->SetTransformParameterObject( parameter_object );
        }
        catch( itk::ExceptionObject & e )
        {
  ...
  if( _parameters.outputLog )
  {
    ...
    if( !_parameters.skipCPU || ( !_parameters.skipCPU && !_parameters.skipGPU ) )
    {
      ...
      interpolatorName = _parameters.interpolator;
